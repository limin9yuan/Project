...
function getGridData(){
    var dataTmp="";
    var url = '/material/purchasePlan/test';
    $.ajax({
        cache : true,
        type : "get",
        url : url,
...
function initPage() {
    ...
    $(window).resize(function (e) {
        window.setTimeout(function () {
            $('#purchasePlanTable').setGridWidth(($('.gridPanel').width()));
            $(".bills").height($(window).height() - 128);
        }, 200);
        e.stopPropagation();
    });
...
function initControl() {
    ...
    /*创建jqGrid的操作按钮容器*/
    /*可以控制界面上增删改查的按钮是否显示*/
    //jQuery("#purchasePlanTable").jqGrid('navGrid', '#requireApplyPage', {edit : false,add : true,del : true});
    //物资名称事件
    ...
    $('.ui-icon-ellipsis').click(function () {
        ...
        layer.open({
            ...
            area : [ '90%', '85%' ],
            content : "/material/purchasePlan/materialList",
            btn: ['确定','取消'],
...
function getMaterialDetailByCode(code){
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            if ( $("#purchasePlanTable").find('[role=row]').find('[data-value=' + result.code + ']').length == 0) {
