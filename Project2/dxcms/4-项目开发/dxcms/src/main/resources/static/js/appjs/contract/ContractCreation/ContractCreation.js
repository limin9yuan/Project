...
//	});
	var options = {
		...
		el : document.querySelector('#sde'), //querySelector满足的值，比如#id、.className、Element。可以是Element，也可以是#id等
...
//			alert(ctrls[i].getValue());
		...
		$.ajax({
			type: 'post',
			url: '/ContractCreation/ContractCreation/test',
			data: {"ctrls":ctrlsArray},
			success: function (r) {
				if (r.code == 0) {
//					layer.msg(r.msg);
				} else {
