...
function selId() {
	...
	if (id==""||id==null) {
		...
		$.ajax({
			cache : true,
			type : "get",
			url : "/material/purchaseOrder/getRequireApplyDetailByCodeId",
			data : {
				'id' : id
			},
			async : false,
			success : function(r) {
				if (r.code == 0) {
					console.log(r.purchaseOrderDetailList);
					var orderEntry = r.purchaseOrderDetailList;
					var $grid = $("#purchaseOrderTable");
					//添加空行
