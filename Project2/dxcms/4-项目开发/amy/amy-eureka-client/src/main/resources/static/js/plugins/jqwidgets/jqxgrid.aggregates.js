	...
	a.extend(a.jqx._jqxGrid.prototype, {
			...
			if (d.aggregatesrenderer) {
				...
				a.each(g, function() {
					...
					for (f in i) {
						...
						var h = f;
						h = e._getaggregatename(h);j.html(h + ":" + i[f]);
						if (e.rtl) {
		...
		_updateaggregates : function() {
			...
			for (var g = 0; g < l; g++) {
				...
				n[n.length] = i[0];this._updatecolumnaggregates(f, h, i)
			}
			if (a.jqx.browser.msie && a.jqx.browser.version < 8) {
				b.css("z-index", m--)
			}
			b.width(parseFloat(e) + 2);b.height(this.statusbarheight);this.statusbar.children().remove();this.statusbar.append(b);this.statusbar.removeClass(this.toThemeProperty("jqx-widget-header"));this.statusbar.addClass(k);this.statusbar.css("border-bottom-color", "transparent");this.statusbar.css("border-top-width", "1px");
