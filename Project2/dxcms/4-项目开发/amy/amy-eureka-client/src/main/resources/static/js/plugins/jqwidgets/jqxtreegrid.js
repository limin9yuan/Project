...
(function(a) {
	a.jqx.jqxWidget("jqxTreeGrid", "jqxDataTable", {});a.extend(a.jqx._jqxTreeGrid.prototype, {
		...
		_showLoadElement : function() {
			...
			if (b.host.css("display") == "block") {
				if (b.autoshowloadelement) {
					a(b.dataloadelement).css("visibility", "visible");a(b.dataloadelement).css("display", "block");b.dataloadelement.width(b.host.width());b.dataloadelement.height(b.host.height())
				}
			}
		},
		_hideLoadElement : function() {
			...
			if (b.host.css("display") == "block") {
				if (b.autoshowloadelement) {
					a(b.dataloadelement).css("visibility", "hidden");a(b.dataloadelement).css("display", "none");b.dataloadelement.width(b.host.width());b.dataloadelement.height(b.host.height())
				}
			}
		},
		getKey : function(b) {
		...
		_renderrows : function() {
				...
				if (this.virtualModeCreateRecords) {
					var am = function(c) {
						if (c === false || (c && c.length == 0)) {
							...
							N.source.hierarchy = new Array();ar._hideLoadElement();N._renderrows();N._updateScrollbars();N._arrange();return
						}
						for (var j = 0; j < c.length; j++) {
							c[j].level = 0;ar.virtualModeRecordCreating(c[j]);
						...
						N._loading = false;ar._hideLoadElement();N._renderrows();N._updateScrollbars();N._arrange()
					};
					N._loading = true;this.virtualModeCreateRecords(null, am);this._showLoadElement()
				}
			}
			if (N.rendering) {
				N.rendering()
			...
			if (O) {
				for (var P = 0; P < b; P++) {
					...
					a(document.body).append(av);var ag = av.find("td");
					k = 1 + parseInt(ag.css("padding-left")) + parseInt(ag.css("padding-right"));av.remove();break
				}
			}
						...
						if (M.aggregates) {
							...
							if (X) {
								if (M.aggregatesrenderer) {
									...
									V = "";
									at[M.displayfield] = "";a.each(X, function() {
										var i = this;
										for (obj in i) {
											var c = obj;
											c = N._getaggregatename(c);var j = '<div style="position: relative; margin: 0px; overflow: hidden;">' + c + ":" + i[obj] + "</div>";
											V += j;
			...
			if (r) {
				...
				aE.css("float", "left");aE.css("pointer-events", "none");aD.css("float", "left");
			...
			if (N._loading && D == 0) {
				N._arrange();this._showLoadElement()
			}
			if (N.rendered) {
				N.rendered()
			}
		...
		checkRow : function(c, d, b) {
			...
			if (f) {
				...
				if (h) {
					...
					e.rowinfo[c].row[g.checked] = true;e._raiseEvent("rowCheck", {
						key : c,
						row : h
					});
					if (b == undefined && this.hierarchicalCheckboxes) {
		...
		uncheckRow : function(c, d, b) {
			...
			if (g) {
				...
				if (i) {
					...
					f.rowinfo[c].row[h.checked] = false;f._raiseEvent("rowUncheck", {
						key : c,
						row : i
					});
					if (b == undefined && this.hierarchicalCheckboxes) {
			...
			if (h) {
				...
				if (this.virtualModeCreateRecords && !h.row._loadedOnDemand) {
					var c = function(k) {
						...
						if (k === false) {
							f._loading = false;e._hideLoadElement();
							h.leaf = true;
							h.row[i.leaf] = true;f._renderrows();return
						}
						for (var l = 0; l < k.length; l++) {
							...
							if (f.rowsByKey[k[l].uid]) {
								f._loading = false;e._hideLoadElement();
								h.leaf = true;
								h.row[i.leaf] = true;f._renderrows();
								throw new Error("Please, check whether you Add Records with unique ID/Key. ")
							}
							f.rowsByKey[k[l].uid] = k[l];e.virtualModeRecordCreating(k[l])
						}
						if (!h.row.records) {
							h.row.records = k
						} else {
						...
						f._renderrows();f._updateScrollbars();
					...
					if (!h.row[i.leaf]) {
						f._loading = true;this._showLoadElement();this.virtualModeCreateRecords(h.row, c);return
					}
				}
				if (!f.updating()) {
					...
					f._renderhorizontalscroll();f._raiseEvent("rowExpand", {
						row : h.row,
						rowKey : d
					})
				}
		...
		collapseRow : function(c) {
			...
			if (f) {
				...
				if (!d.updating()) {
					...
					d._renderrows();d._updateScrollbars();
					var e = b != d.vScrollBar.css("visibility");
					if (d.height === "auto" || d.height === null || d.autoheight || e) {
						d._arrange()
					}
					d._renderhorizontalscroll();d._raiseEvent("rowCollapse", {
						row : f.row,
						rowKey : c
					})
				}
