...
(function(h, q, i) {
	...
	function j(J, H, F, E, I, K) {
		if (F.jqxSettings) {
			...
			if (!G.apply) {
									...
									h.each(N, function(Q, U) {
										...
										if (!p[F.jqxSettings + "." + Q]) {
											...
											var R = I.$watch(F.jqxSettings + "." + Q, function(X, W) {
												if (X != W) {
													var V = {};
													V[T] = X;h.each(D[I.$id + E], function(Z, aa) {
				...
				D[I.$id + E] = new Array();D[I.$id + E].push(K)
			} else {
				if (!D[I.$id + E]) {
					D[I.$id + E] = new Array()
				}
	...
	function n(O, I, N, E, M) {
						...
						if (Z != X) {
							...
							if (Y == "watch") {
								if (N.jqxWatch.indexOf(",") >= 0 || N.jqxWatch.indexOf("[") >= 0) {
									...
									ab = ab.replace("]", "");
									ab = ab.trim();
									ab = ab.split(",");h.each(ab, function(ae, ag) {
										var ah = this.split(".");
		...
		});k(O, I, N, E, M, J);var L = I[0];
		j(O, I, N, E, M, L);var H = h(I)[E](K);
		var P = h(I)[E]("getInstance");
		B(O, I, N, E, M, G, L, P, K);O.$on("$destroy", function() {
			if (P && P.destroy) {
				h(I)[E]("destroy")
			} else {
				h(I).remove()
	...
	function w(H) {
		...
		if (L.indexOf("jqx") >= 0) {
					...
					if (L.indexOf("jqx-toggle-button") >= 0) {
						...
						if (L.indexOf("jqx-link-button") >= 0) {
							if (h(H).find("a").length > 0) {
								var I = h(H).find("a");
								I.attr("id", "jqx-ngwidget");h(H).replaceWith(I)
							} else {
								...
								if (L.indexOf("jqx-list-box") >= 0 || L.indexOf("jqx-drop-down-list") >= 0 || L.indexOf("jqx-combo-box") >= 0) {
									...
									if (L.indexOf("jqx-list-menu") >= 0) {
										...
										if (L.indexOf("jqx-tooltip") >= 0) {
											var E = h(H).children();
											E.detach();h(E).insertAfter(h(H));h.each(G, function() {
												if (h(E)[0]) {
			...
			K = M.find("#jqx-ngwidget").removeAttr("id");h.each(G, function() {
				if (h(K)[0]) {
	...
	function t(G, H) {
		function F(L) {
			...
			if (!L.scripts) {
				...
				J(L.scripts);h.each(L.deps, function(Q, P) {
					var O = new Array();
					O.push(Q);J(O, function() {
						J(P)
					})
				})
			}
