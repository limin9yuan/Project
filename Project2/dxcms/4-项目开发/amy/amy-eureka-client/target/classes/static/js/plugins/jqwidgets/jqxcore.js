...
(function(a) {
	...
	a.jqx.define = function(b, c, d) {
		b[c] = function() {
			if (this.baseType) {
				this.base = new b[this.baseType]();this.base.defineInstance()
			}
			this.defineInstance()
		};
	...
	a.jqx.jqxWidgetProxy = function(g, c, b) {
		...
		if (a.jqx.isPropertySetter(e, b)) {
			if (a.jqx.validatePropertySetter(e, b)) {
				a.jqx.set(e, b);return undefined
			}
		} else {
	...
	a.jqx.applyWidget = function(c, d, k, l) {
		...
		l.widgetName = d;a.data(c, d, j);a.data(c, "jqxWidget", j.instance);
		var h = new Array();
		var l = j.instance;
		while (l) {
			l.isInitialized = false;h.push(l);
			l = l.base
		}
		h.reverse();
		h[0].theme = a.jqx.theme || "";a.jqx.jqxWidgetProxy(d, c, k);
		for (var b in h) {
			l = h[b];
			if (b == 0) {
				l.host = m;
	...
	a.jqx.jqxWidget = function(b, c, f) {
								...
								set : function(B) {
									if (this.widgetInstance && this.widgetInstance[z] != B) {
										if (this.widgetInstance[z] != B && d(this.widgetInstance[z]) != d(B)) {
											var A = {};
											A[z] = B;this.widgetInstance.host[g](A);
		...
		} catch (h) {} a.extend(a.jqx["_" + g].prototype, {
		...
		for (var b = 0; b < c.length; b++) {
			...
			if (window.addEventListener) {
				...
				case "mousewheel":
					...
					continue;case "mousemove":
					if (!f) {
						g[0].addEventListener("mousemove", e, false);continue
					}
					break
				}
			}
		...
		for (var b = 0; b < c.length; b++) {
			...
			if (window.removeEventListener) {
				...
				case "mousewheel":
					...
					continue;case "mousemove":
					if (e) {
						f[0].removeEventListener("mousemove", e, false);continue
					}
					break
				}
			}
	...
	a.jqx.utilities = a.jqx.utilities || {};a.extend(a.jqx.utilities, {
		...
		setTheme : function(f, g, e) {
			...
			for (var c = 0; c < h.length; c += 1) {
				if (h[c].indexOf(f) >= 0) {
					if (f.length > 0) {
						b.push(h[c]);j.push(h[c].replace(f, g))
					} else {
						j.push(h[c].replace("-" + g, "") + "-" + g)
					}
				}
			}
			this._removeOldClasses(b, e);this._addNewClasses(j, e);
		...
		resize : function(d, m, l, k) {
			...
			if (this.resizeHandlers) {
				...
				if (l === true) {
					...
					if (this.resizeHandlers.length == 0) {
						var j = a(window);
						if (j.off) {
							j.off("resize.jqx");j.off("orientationchange.jqx");j.off("orientationchanged.jqx")
						} else {
							j.unbind("resize.jqx");j.unbind("orientationchange.jqx");j.unbind("orientationchanged.jqx")
						}
			...
			var h = function(p, x) {
				...
				f.hiddenWidgets = new Array();f.resizeHandlers.sort(o);
			...
			if (!this.resizeHandlers) {
				...
				var j = a(window);
				if (j.on) {
					this._resizeTimer = null;j.on("resize.jqx", function(n) {
	...
	a.jqx.position = function(b) {
				...
				var f = function() {
					for (var g in d) {
						if (c.addEventListener) {
							c.removeEventListener(g, b._touchListener);c.addEventListener(g, b._touchListener, false)
						}
					}
				};
					...
					if (d.hasOwnProperty(i)) {
						...
						if (match) {
							if (i.toString() == "blackberry") {
								this.touchDevice = false;return false
							}
							this.touchDevice = true;return true
						}
					}
				}
		...
		isNewSafariMobileBrowser : function() {
			...
			b = b && (c.indexOf("version/5") != -1);return b
		},
		isSafari4MobileBrowser : function() {
			var c = navigator.userAgent.toLowerCase();
			var b = c.indexOf("ipad") != -1 || c.indexOf("iphone") != -1 || c.indexOf("ipod") != -1;
			b = b && (c.indexOf("version/4") != -1);return b
		},
		...
		touchScroll : function(d, y, g, D, b, k) {
			...
			var x = function(J) {
				...
				if (G || K) {
					if ((n !== 0 && G) || (l !== 0 && K)) {
						g(-n * 1, -l * 1, F, E, J);J.preventDefault();J.stopPropagation();
			...
			if (this.simulatetouches) {
				...
				var w = function(E) {
					...
					B.dispatchMouseEvent("mouseup", G, F);B.dispatchMouseEvent("click", G, F)
				};
			...
			var f = function(E) {
				...
				if (h) {
					...
					var G = B.getTouches(E)[0],
						F = B.getRootNode(G.target);
					B.dispatchMouseEvent("mouseup", G, F);B.dispatchMouseEvent("click", G, F);return true
				}
			};
			if (q.on) {
	...
	a.jqx.cookie = a.jqx.cookie || {};a.extend(a.jqx.cookie, {
		cookie : function(e, f, c) {
			if (arguments.length > 1 && String(f) !== "[object Object]") {
				...
				f = String(f);return (document.cookie = [ encodeURIComponent(e), "=", c.raw ? f : encodeURIComponent(f), c.expires ? "; expires=" + c.expires.toUTCString() : "", c.path ? "; path=" + c.path : "", c.domain ? "; domain=" + c.domain : "", c.secure ? "; secure" : "" ].join(""))
			}
			c = f || {};
			var b,
				g = c.raw ? function(j) {
	...
	a.jqx.string = a.jqx.string || {};a.extend(a.jqx.string, {
		...
		endsWith : function(b, c) {
			...
			b = this.normalize(b);return b.slice(-c.length) == c
		},
		endsWithIgnoreCase : function(b, c) {
			if (b == null || c == null) {
				return false
			}
			b = this.normalize(b);return b.toUpperCase().slice(-c.length) == c.toUpperCase()
		}
	});a.extend(a.easing, {
