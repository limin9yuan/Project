...
function initControl() {
	...
	$grid.jqGrid(
		{
			...
			colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
				...
				{
					name : 'materialUnitId',
					index : 'materialUnitId',
					width : 60,
					align : "right"
				}, //单位
				{
					name : 'materialSubArray',
					index : 'materialSubArray',
					width : 80,
					align : "right"
				}, //包装物资
	...
	$grid.jqGrid('setGroupHeaders', {
		...
		groupHeaders : [
			{
				startColumnName : 'stockQty',
				numberOfColumns : 3,
				titleText : '<div class="group-headers">数量信息</div>'
			},
			{
				startColumnName : 'budgetPrice',
				numberOfColumns : 4,
				titleText : '<div  class="group-headers">金额信息</div>'
			}
		]
	});
...
function calculateTotal() {
	//合计
	var budgetTotalQty = 0.00,
		budgetTotalAccount = 0.00,
		requireTotalQty = 0.00,
		referenceTotalAccount = 0.00;
...
function getGridData() {
	...
	$("#requireApplyTable").find('[role=row]').each(function(i) {
		if (!!$(this).find('input[name="materialName"]').val()) {
			applyEntryJson.push({
				name : $(this).find('input[name="materialName"]').val(), //物资名称
				code : $(this).find('input[name="materialCode"]').val(), //物资编码
	...
	for (var i = 0; i < data.length; i++) {
		...
		$grid.find('[role=row]').each(function (i) {
			...
			if (result != undefined ) {
				...
				$(this).find('input[name="materialCode"]').val(result.materialCode).attr('data-value', result.code);
				...
				$(this).find('input[name="referenceTotal"]').val(result.referenceTotal);
