								...
								},
								{
									...
									formatter : function(value, row, index) {
										var e = '<a class="btn btn-primary btn-sm '
												+ s_edit_h
												+ '" href="#" mce_href="#" title="编辑" onclick="editField(\''
												+ row.id
												+ '\')"><i class="fa fa-edit"></i></a> ';
										var d = '<a class="btn btn-warning btn-sm '
												+ s_remove_h
												+ '" href="#" title="删除"  mce_href="#" onclick="removeField(\''
												+ row.id
...
function reloadParenWindow() {
	// 选项卡菜单已存在
	$('.J_menuTab', window.parent.document).each(
			function() {
				if ($(this).hasClass('active')) {
					var dataUrl = $(this).data('id');
					$('.J_mainContent .J_iframe', window.parent.document).each(
							function() {
