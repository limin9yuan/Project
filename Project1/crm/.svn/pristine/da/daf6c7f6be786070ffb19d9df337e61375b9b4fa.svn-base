						...
						columns : [
								{
									...
									formatter : function(value, row, index) {
										var e = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="编辑" onclick="editPurchase(\''
												+ row.purchaseId
												+ '\')"><i class="fa fa-edit"></i></a> ';
										var d = '<a class="btn btn-warning btn-sm '+s_remove_h+'" href="#" title="删除"  mce_href="#" onclick="removePurchase(\''
												+ row.purchaseId
...
function addPurchase() {
	layer.open({
		...
		area : [ '800px', '95%' ],
		//content : prefixPurchase + '/add' // iframe的url
		content : prefixPurchase + '/add/'+$('#budgetId').val() // iframe的url
	});
}
function editPurchase(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '95%' ],
		content : prefixPurchase + '/edit/' + id // iframe的url
	});
}
...
function batchRemovePurchase() {
	...
	// 按钮
		...
		$.ajax({
			...
			url : prefixPurchase + '/batchRemove',
			success : function(r) {
				if (r.code == 0) {
					layer.msg(r.msg);
					reLoadPurchase();
