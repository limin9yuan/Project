...
public class SalesProjectServiceImpl implements SalesProjectService {
	...
	public int save(SalesProjectDO salesProject){
		//salesProject.getProjectCreateTime();
		//salesProject.setProjectOperateTime(new Date());
		//salesProject.getProjectBeginDate();
		//salesProject.getProjectEndDate();
		//salesProject.setProjectBeginDate(new Date());
		//salesProject.setProjectEndDate(new Date());
		int r = salesProjectDao.save(salesProject);
		return r;
		// 保存到接受者列表中
		/*Long[] userIds = salesProject.getUserIds();
		Long notifyId = salesProject.getId();
		List<SalesProjectDO> records = new ArrayList<>();
		for (Long userId : userIds) {
			SalesProjectDO record = new SalesProjectDO();
			record.setNotifyId(notifyId);
			record.setUserId(userId);
			record.setIsRead(0);
			records.add(record);
		}
		salesProjectDao.batchSave(records);*/
		...
		try {
			...
			my: for (int i = 0; i < rows; i++) {
				...
				if (row != null) {
					...
					salesProjectDO.setProjectOperator(userid); 
					...
					rtn = salesProjectDao.save(salesProjectDO);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
