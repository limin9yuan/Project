var prefixreceived = "/sales/companyCustomer"
$(function() {
	loadreceivedNo();
});

function loadreceivedNo() {
	$('#noReturnMoneyTable')
			.bootstrapTable(
					{
						method : 'get', // 服务器数据的请求方式 get or post
						url : prefixreceived + "/examineNonPaymentEnterprise", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
...
function examineCompanyCustomer(id) {
	parent.layer.open({
		...
		area : [ '95%', '95%' ],
		content : prefixreceived + '/examine/' + id
	});
}
