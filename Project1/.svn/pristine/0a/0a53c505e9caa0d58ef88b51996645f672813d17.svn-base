		...
		try {
			...
			if (list != null && !list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					// 操作时间
					String getCustomerCreateTime = "";
					if (report.getCustomerCreateTime() != null) {
						getCustomerCreateTime = sdf.format(report.getCustomerCreateTime());
					}
					hssfRow.createCell(63).setCellValue(getCustomerCreateTime);
				}
			}
			// 第七步，将文件输出到客户端浏览器
			try {
