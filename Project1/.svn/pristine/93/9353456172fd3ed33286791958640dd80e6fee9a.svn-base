...
//                .generateDiagram(bpmnModel, "png", activeActivityIds);
    ...
    //判断流程是否结束
    public boolean isProcessInstanceFinish(String pProcessInstanceId) {
    	
    	long cou=historyService.createHistoricProcessInstanceQuery().processInstanceId(pProcessInstanceId).finished().count();
    	if(cou>0){ //结束
    		return true;
    	}else{ //未结束
    		return false;
