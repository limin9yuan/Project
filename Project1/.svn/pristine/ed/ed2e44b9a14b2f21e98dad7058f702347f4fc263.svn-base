...
function customerContactMapper_edit(){
	$.ajax({
		...
		success : function(data) {
			...
			$("input[name='contactResponsibility']").val(result.contactResponsibility);
			//纪念日类别
			$("input[name='contactSpecialDayCategory']").val(result.contactSpecialDayCategory);
			//纪念日
...
//			$("input[name='contactRemarks']").val(result.contactRemarks);
			...
			loadCrmDataValue("/inner/innerOrgEmployee/listDic","contactOwner",result.contactOwner);//客户所有者
			loadDicValue("sales_Customer_Contact_Status","contactStatus",result.contactStatus);//联系人状态
			loadCrmDataValue("/inner/innerOrgEmployee/listDic","contactSales",result.contactSales);//销售负责人  
