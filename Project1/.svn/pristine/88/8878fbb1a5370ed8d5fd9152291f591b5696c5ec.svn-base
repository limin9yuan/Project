			...
			.bootstrapTable(
					{
						...
						url : childPrefix + "/list", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						columns : [
								...
								},
								{
									...
									title : '子公司名称' 
																...
																{
									align : 'center',
									field : 'childCompanyRemarks', 
									title : '备注' 
								},
																{
									align : 'center',
									field : 'childCompanyCreateTime', 
									title : '创建时间' 
								},
																{
									align : 'center',
									field : 'childCompanyOperateTime', 
									title : '修改时间' 
								},
																{
									align : 'center',
									field : 'childCompanyOperator', 
									title : '操作人' 
								}
															]
					});
...
function editChild(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '95%' ],
		content : childPrefix + '/edit/' + id // iframe的url
...
function batchRemoveChild() {
	var rows = $('#childTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
