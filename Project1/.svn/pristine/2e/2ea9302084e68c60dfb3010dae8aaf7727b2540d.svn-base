			...
			.bootstrapTable(
					{
						...
						url : competitorPrefix + "/list", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
...
function reLoadCompetitor() {
	$('#competitorTable').bootstrapTable('refresh');
}
function addCompetitor() {
	layer.open({
		...
		area : [ '800px', '95%' ],
		content : competitorPrefix + '/add/'+$('#customerId').val() // iframe的url
	});
}
...
function batchRemoveCompetitor() {
	var rows = $('#competitorTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
	...
	// 按钮
		...
		$.ajax({
			...
			url : competitorPrefix + '/batchRemove',
			success : function(r) {
				if (r.code == 0) {
					layer.msg(r.msg);
					reLoadCompetitor();
