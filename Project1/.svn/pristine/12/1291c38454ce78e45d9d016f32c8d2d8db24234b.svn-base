...
public class ContractServiceImpl implements ContractService {
	...
	public int save(ContractDO contract) {
		...
		actTaskService.startProcess(ActivitiConstant.ACTIVIYI_CONTRACT[0],
				ActivitiConstant.ACTIVIYI_CONTRACT[1], contractId,contract.getContractName(), new HashMap<String,Object>());
				...
				if (row != null) {
					...
					for (int j = 0; j < cellCount; j++) {
						...
						if (j == 0) {
							...
							contractDO.setContractHardwareList(cellvalue);
						} else if (j == 14) {
							contractDO.setContractSoftwareList(cellvalue);
						} else if (j == 15) {
	...
	public int formUpdate(ContractDO contract){
		...
		actTaskService.complete(contract.getTaskId(),vars);
