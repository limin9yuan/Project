...
public class AdditionalRecordsServiceImpl implements AdditionalRecordsService {
	...
	@Override
	public AdditionalRecordsDO view(String recordId) {
		return additionalRecordsDao.view(recordId);
	}
	...
	public int save(AdditionalRecordsDO additionalRecords) {
		...
		String additionalRecordsId = additionalRecords.getRecordId();
		//流程标题，每个业务根据自己特点，体现主要信息
		String title = "";
		title = additionalRecords.getRecordId() + "-" + additionalRecords.getRecordName();
		//添加保存时发起审批流程
		String PROCESS_INSTANCE_ID = actTaskService.startProcess(ActivitiConstant.ACTIVITI_CONTRACT_ADDITIONALRECORDS[0],
				ActivitiConstant.ACTIVITI_CONTRACT_ADDITIONALRECORDS[1], additionalRecordsId, title, new HashMap<String, Object>());
		//更新流程实例ID到业务表
		additionalRecords.setProcessInstanceId(PROCESS_INSTANCE_ID);
		additionalRecordsDao.update(additionalRecords);

		return Integer.parseInt(additionalRecordsId);
	}
			...
			for (int i = 0; i < rows; i++) {
				...
				if (row != null) {
					...
					for (int j = 0; j < cellCount; j++) {
						...
						if (j == 0) {
							additionalRecordsDO.setContractId(cellvalue);
						} else if (j == 1) {
							additionalRecordsDO.setRecordName(cellvalue);
						} else if (j == 2) {
							additionalRecordsDO.setRecordBulidCompany(cellvalue);
						} else if (j == 3) {
							additionalRecordsDO.setRecordTotalPrice(new BigDecimal(cellvalue));
						} else if (j == 4) {
					...
					rtn = additionalRecordsDao.saveDownloadTemplate(additionalRecordsDO);
				}
			}
		} catch (Exception e) {
	...
	@Override
	public int updateRecordAttachment(AdditionalRecordsDO additionalRecords) {
		// TODO Auto-generated method stub
		return additionalRecordsDao.updateRecordAttachment(additionalRecords);
	}

	@Override
	public int saveDownloadTemplate(AdditionalRecordsDO additionalRecords) {
		// TODO Auto-generated method stub
		return additionalRecordsDao.saveDownloadTemplate(additionalRecords);
	}
}
