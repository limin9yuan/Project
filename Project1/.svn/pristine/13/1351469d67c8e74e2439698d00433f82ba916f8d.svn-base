...
$(function() {
	loadCrmData("/sales/salesProject/listAllDic","projectName");
	loadreceived();
	datetimepicker();
	validateRule();
});
...
function loadreceived() {
						...
						columns : [
								{
									...
									formatter : function(value, row, index) {
										var e = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="编辑" onclick="editreceived(\''
												+ row.receivedId
	...
	// 导入弹出层
        ...
        $("div[name='received_window'] button[name='tab_excelinsertQuitbtn']").click(function () {
            $("div[name='received_window']").modal('hide')
        })
}
function reLoad() {
	$('#receivedTable').bootstrapTable('refresh');
}
...
function view(id) {
	parent.layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '95%', '95%' ],
		content : prefixreceived + '/view/' + id // iframe的url
	});
}
...
function batchRemovereceived() {
	var rows = $('#receivedTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
	...
	// 按钮
		...
		$.ajax({
			...
			url : prefixreceived + '/batchRemove',
			success : function(r) {
				if (r.code == 0) {
					layer.msg(r.msg);
					reLoad();
