...
public class ArmTagSupportServiceImpl extends BaseServiceImpl implements ArmTagSupportService {
	...
	public Dto getCardList(Dto pDto) {
		...
		if (!accountType.equalsIgnoreCase(ArmConstants.ACCOUNTTYPE_NORMAL)) {
			resultList = g4Dao.queryForList("getCardListBasedSuperAndDeveloper", pDto);
			outDto.setDefaultAList(resultList);
			return outDto;
		}
		List cardListBasedRole = g4Dao.queryForList("getCardList", pDto);
		List cardListBasedUser = g4Dao.queryForList("getCardListBasedUser", pDto);
	...
	public Dto getCardTreeList(Dto pDto) {
		...
		if (!accountType.equalsIgnoreCase(ArmConstants.ACCOUNTTYPE_NORMAL)) {
			if (G4Utils.isOracle()) {
				resultList = g4Dao.queryForList("getCardTreeListBasedSuperAndDeveloper", pDto);
			} else if (G4Utils.isMysql()) {
				resultList = g4Dao.queryForList("getCardTreeListBasedSuperAndDeveloperMysql", pDto);
			}
			outDto.setDefaultAList(resultList);
			return outDto;
		}
		...
		if (G4Utils.isOracle()) {
			cardTreeListBasedRole = g4Dao.queryForList("getCardTreeList", pDto);
		} else if (G4Utils.isMysql()) {
			cardTreeListBasedRole = g4Dao.queryForList("getCardTreeListMysql", pDto);
		}
		List cardTreeListBasedUser = new ArrayList();;
		if (G4Utils.isOracle()) {
			cardTreeListBasedUser = g4Dao.queryForList("getCardTreeListBasedUser", pDto);
		} else if (G4Utils.isMysql()) {
			cardTreeListBasedUser = g4Dao.queryForList("getCardTreeListBasedUserMysql", pDto);
		}
	...
	public Dto getDepartmentInfo(Dto pDto) {
		Dto outDto = (BaseDto) g4Dao.queryForObject("getDepartmentInfo", pDto);
		String deptname = ((BaseDto)g4Dao.queryForObject("getDepartmentInfo", pDto)).getAsString("deptname");
		return outDto;
	}
	...
	public Dto getEauserSubInfo(Dto pDto) {
		return (BaseDto)g4Dao.queryForObject("getEauserSubInfo", pDto);
	}
