...
Ext.onReady(function() {
		   //统计分项
		    var rptStore = new Ext.data.SimpleStore({
						fields : ['text', 'value'],
						data : [['按用户统计', 'c01050201'], ['按小区统计', 'c01050201_1'], ['按换热站统计', 'c01050201_2'],['按收费员统计', 'c01050201_3']]
					});			
		   ...
		   item_codeCbx = new Ext.form.ComboBox({
			...
			function fnSumInfo() {
				...
				Ext.Ajax.request({
							...
							params:{p_range:strID,
									...
									charge_month : Ext.getCmp('charge_month').getValue(),
									item_code : Ext.getCmp('item_code').getValue(),
									end_date: Ext.getCmp('end_date').getRawValue()+" 23:59:59"
									}
						});
			}
			/**
			...
			function printHtml() {
				...
				//收费截止时间
				var v_end_date = Ext.getCmp('end_date').getRawValue()+" 23:59:59";
				if(v_end_date==""){
					v_end_date="-1"
				}
				...
				if(rpt_id!='c01050201'){
					...
					Ext.Ajax.request({
							...
							success : function(response) { // 回调函数有1个参
								...
								var strurl = "../module/rpt/htmlView.jsp?rpt_id="+rpt_id
