...
Ext.onReady(function() {
			...
			store = new Ext.data.Store({
						...
						proxy : new Ext.data.HttpProxy({
									url : 'rlb.ered?reqCode=queryChargeDetail'
								}),
						// 数据读取器
			...
			function rollbackClick(){
				...
				if(grid.store.getTotalCount()==0){
					Ext.MessageBox.alert('提示', '没有可冲账的记录!');
					return;
				}
				var record = grid.getSelectionModel().getSelected();
				if (Ext.isEmpty(record)) {
					Ext.Msg.alert('提示', '请先选择您要冲账的记录');
					return;
				}
				if(record.get('rollback_flag')=='已冲账'){
					Ext.MessageBox.alert('提示', '该记录已经被冲账,请勿重复冲账!');
					return;
				}	
				...
				//添加小区表单
					rollbackForm = new Ext.form.FormPanel({
						region : 'center',
						title : '',
						collapsible : false,
						border : false,
					...
					rollbackWindow = new Ext.Window({
						...
						buttons : [{ // 窗口底部按钮配置
							...
							handler : function() { // 按钮响应函数
								...
								rollbackForm.getForm().loadRecord(record);
								Ext.getCmp('btnRollback').enable();
							}
						},{ // 窗口底部按钮配置
							text : '关闭', // 按钮文本
		...
		function rollback() {
			...
			Ext.MessageBox.confirm('请确认', '您确定要冲账吗?',function(btn, text) {
					if (btn == 'yes') {	
						Ext.getCmp('btnRollback').disable();
						var params={
						b_id:strChecked,
						invalid_invoice:'1',
						rollback_reason:rollbackForm.getForm().findField("rollback_reason").getValue(),
						rollback_remark:rollbackForm.getForm().findField("rollback_remark").getValue()
						};
						Ext.Ajax.request({
							url : 'rlb.ered?reqCode=rollback',
