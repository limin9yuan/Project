...
public class CommonUtils {
	    //构造器
	    public CommonUtils(){}
	    //方法changeToBig()转换数字为大写金额的字符
	    public String getNumberBig(String value)
	    {
	    	if(value.equals("null")){
	    		...
	    		return numberToBig(Double.parseDouble(value));
	    	}
	    }
	    public     String    numberToBig(double  value){ 
	        ...
	        String   flag=valStr.substring(0, 1) ;
	        String   prefix="";    //整数部分转化的结果   
	        String   suffix="";    //小数部分转化的结果 
	        String   Flag = "";    //是否为负数
	        if(null!=flag && !"".equals(flag) && flag.trim().equals("-")){       	
	        	Flag="负";
	            head=valStr.substring(1,valStr.length()-2);
	        }
	        //处理小数点后面的数   
	        ...
	        for(int   i=0;i<chDig.length;i++){                               //循环处理每个数字   
	          ...
	          return   Flag+prefix+suffix+"整";                //返回正确表示   
	    }  
	    //测试hangeToBig方法
	    public   static   void   main(String   args[]){   
	    	CommonUtils   a=new   CommonUtils();   
	        System.out.println("a.numberToBig(22233482.74)= "+a.getNumberBig("22233482.74"));
