 ...
 * @since 2011-05-14
 */
public class MyArmViewportTag extends TagSupport {
	...
	private String getPanelScript() {
		...
		String treesString = generateMainMenus(dto);
		return treesString + "\n" + writer.toString();
	}
	 ...
	 * @param rootMenuId
	 */
	private String generateMainMenus(Dto pDto) {
		...
		String treesString = scriptStart 
		+ "var tmpbtns='';"+"\n"+" var j_buttons;"
		...
		String rootName = (String) sqlRunner.queryForObject("getMenuNameForCNPath", "01");
		...
		TemplateEngine engine = TemplateEngineFactory.getTemplateEngine(TemplateType.VELOCITY);
		DefaultTemplate template = new FileTemplate();
		template.setTemplateResource(TagHelper.getTemplatePath(getClass().getName(), "MainMenuTag.tpl"));
		StringWriter writer = engine.mergeTemplate(template, dto);
